<script>
	...
	async function handleCalendarSubmit(event) {
		...
		try {
			...
			await saveCalendarEvent(eventContent);
			...
			error = `Error creating calendar event: ${err.message}`;
		} finally {
			loading = false;
			setTimeout(() => {
				success = null;
	...
	async function deleteCalendarEvent(eventId) {
		try {
			...
			error = `Error deleting calendar event: ${err.message}`;
		}
	}
		...
		<div class="relative z-10">
			<div class="p-4 max-w-6xl mx-auto">
				...
				<div class="mb-8 bg-white p-6 rounded-lg shadow-md">
					<h3 class="text-2xl font-bold text-orange-500 mb-4">New Calendar Event</h3>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<input
									...
									id="eventLocation"
									required
									bind:value={calendarEvent.location}
									class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500"
								/>
							</div>
							<div>
				...
				<!-- Calendar Events -->
				<div class="mb-8">
					<h3 class="text-2xl font-bold text-orange-500 mb-4">Calendar Events</h3>
					{#if calendarEvents.length === 0}
						<p class="text-center text-gray-500 py-8">No calendar events created yet</p>
					{:else}
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-blue-500">
									<div class="flex justify-between items-start mb-2">
										...
										<button
											...
											disabled={loading}
											class="px-3 py-1 text-sm text-white bg-red-500 rounded hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500"
										>
											Delete
										</button>
									</div>
									<p class="text-gray-700 mb-2 whitespace-pre-wrap">{event.description}</p>
									...
									<div class="text-sm text-gray-500">
										...
										<p>Created at: {event.timestamp}</p>
									</div>
								</div>
							{/each}
						</div>
